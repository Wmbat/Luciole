...
namespace ui
{
   ...
   window::window( window::create_info_t const& create_info )
      :
      title( create_info.value( ).title ),
      position( create_info.value( ).position ),
      size( create_info.value( ).size ),
      is_wnd_open( true ),
      is_fullscreen( false )
   {
      ...
      window_logger = std::shared_ptr<spdlog::logger>( 
         new spdlog::logger( "Window Logger", { console_sink } ) 
      ); 

#if defined( VK_USE_PLATFORM_XCB_KHR )
      window_logger->info( 
         "Using XCB for window creation"
      );
   
      /** Connect to X11 window system. */
      p_xcb_connection = xcb_connection_uptr(
         xcb_connect( nullptr, &default_screen_id ),
         []( xcb_connection_t* p ) { xcb_disconnect( p ); } );

      if( xcb_connection_has_error( p_xcb_connection.get() ) )
      {
         window_logger->error(
            "Failed to connecte to the X server."
      ...
      {          
         window_logger->info(
            "Connection to X server established"
         );
      }
      ...
      window_logger->info(
         "Window ID generated: {0}.", std::to_string( xcb_window )
      );
      ...
      xcb_create_window(
         p_xcb_connection.get(),             /* Connection             */
      ...
      xcb_change_property(
         p_xcb_connection.get(), XCB_PROP_MODE_REPLACE,
         xcb_window, p_reply->atom, 4, 32, 1,
         &p_xcb_wm_delete_window->atom 
      );

      /** Change the title of the window. */
      xcb_change_property(
         p_xcb_connection.get(), XCB_PROP_MODE_REPLACE,
         xcb_window, XCB_ATOM_WM_NAME, XCB_ATOM_STRING, 8,
      ...
      if ( is_fullscreen )
      {
         ...
         xcb_change_property(
            p_xcb_connection.get(), XCB_PROP_MODE_REPLACE,
            xcb_window, p_atom_wm_state->atom,
            XCB_ATOM_ATOM, 32, 1,
      ...
      while( ( e = xcb_poll_for_event( p_xcb_connection.get() ) ) )
      {
         switch ( e->response_type & 0x7f )
         {
            ...
            case XCB_CONFIGURE_NOTIFY:
            {
               ...
               if ( size.x != static_cast<uint32_t>( motion_event->width ) &&
   ...
   std::variant<VkSurfaceKHR, vk::error> window::create_surface( 
      vk::instance_t instance ) const
   {
#if defined( VK_USE_PLATFORM_XCB_KHR )
      ...
      vk::error const err( vk::result_t(
         vkCreateXcbSurfaceKHR( instance.value( ), &create_info, nullptr, &surface )
      ) );
