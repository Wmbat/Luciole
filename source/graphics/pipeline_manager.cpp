...
namespace twe
{
    ...
    std::vector<pipeline::id> pipeline_manager::insert( pipeline_create_info& create_info )
    {
        ...
        std::vector<vk::PipelineInputAssemblyStateCreateInfo> input_assembly_create_infos( size );
        std::vector<vk::Viewport> viewports( size );                           // TODO: allow for multiple viewports.
        std::vector<vk::Rect2D> scissors( size );                              // TODO: allow for multiple scissors.
        std::vector<vk::PipelineViewportStateCreateInfo> viewport_create_infos( size );
        std::vector<vk::PipelineRasterizationStateCreateInfo> rasterization_create_infos( size );
        std::vector<vk::PipelineMultisampleStateCreateInfo> multisample_create_infos( size );
    
        std::vector<std::vector<vk::PipelineColorBlendAttachmentState>> attachments( size );
        std::vector<vk::PipelineColorBlendStateCreateInfo> colour_blend_create_infos( size );
    
        std::vector<std::vector<vk::DynamicState>> dynamic_states( size );
        std::vector<vk::PipelineDynamicStateCreateInfo> dynamic_state_create_infos( size );
