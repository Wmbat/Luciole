...
namespace TWE
{
    ...
    private:
        const vk_return_type<VkInstance> create_instance( const std::vector<const char*>& extensions,
                                                          const std::vector<const char*>& debug_layers,
                                                          const std::string& app_name, uint32_t app_version ) const noexcept;

        const vk_return_type<VkDebugReportCallbackEXT> create_debug_report( const VkInstance& instance ) const noexcept;

        const vk_return_type<VkSurfaceKHR> create_surface( const window& wnd, const VkInstance& instance ) const noexcept;

        const vk_return_type<VkPhysicalDevice> pick_physical_device( const VkSurfaceKHR& surface,
