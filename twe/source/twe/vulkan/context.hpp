...
namespace twe
{
    namespace vulkan
    {
        ...
        public:
            context( ) = default;
            context( base_window *p_window, const std::string& app_name, uint32_t app_version )
            {
            ...
            std::enable_if_t<std::is_same_v<C, vk::UniqueDevice>, C> create_handle( const std::vector<queue_family_info>& queue_family_infos ) const noexcept
            {
                ...
                for ( const auto& info : queue_family_infos )
                {
                    if( info.is_dedicated_transfer() )
                    {
                        ...
                        for( size_t i = 0; i < info.count_; ++i )
                        {
        ...
        public:
            ...
            std::array<vk::UniqueCommandPool, frame_count> graphics_command_pools_;
